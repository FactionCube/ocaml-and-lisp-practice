...
let nxt = Iter.(of_str raw |> map (fun c -> HexCraft.hexchar2int c) |> to_list |> HexCraft.combine )
;;
...
Iter.(of_list nxt |> map (fun c -> HexCraft.int2hexchar c ) |> to_str) ;;
(* - : string = "\02777316?x\021\027\127+x413=x9x(7-6<x7>x:9;76" *)

let alst = Iter.(of_list nxt |> map (fun c -> HexCraft.int2hexchar c ) |> to_list) ;;
(* val alst : char list =
...
Iter.(of_list nxt |> map (fun c -> c lxor 0x58) |> map (fun c -> HexCraft.int2hexchar c ) |> to_str) ;;
